(ns hive-qdrant.lifecycle
  "IShutdownHook for the Qdrant client. Priority 210 (client band).
   ...
   Registers itself with hive-spi.lifecycle.registry at ns-load time via a
...
(defrecord QdrantShutdown []
  ...
  (shutdown!         [_ _ctx]
    (try
      ...
      ;; via the hive-spi memory registry — best-effort, never throws.
      (if-let [close-fn (requiring-resolve 'hive-qdrant.store/close!)]
        (do (close-fn)
            (log/info "qdrant shutdown complete"))
        (let [disconnect! (requiring-resolve 'hive-spi.memory.ports/disconnect!)
              get-store   (requiring-resolve 'hive-spi.memory.registry/get-store)]
...
(defn install! []
  (reg/register-shutdown! "qdrant/close" (->QdrantShutdown)))
